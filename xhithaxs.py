...
""")
    url = input("Masukan url : ")
    wordlist = ("main/wordlist.txt")

    lists = open(wordlist, "r")

    list = lists.readlines()

    for a in list:
        text = a.strip()
        re = req.get(f"{url}/{text}")
        if re.status_code == 200:
            print (Fore.GREEN, f"{url}/{text} Status code : {re.status_code}")
        elif re.status_code == 404:
            print (Fore.RED ,f"{url}/{text} Status code : {re.status_code}")
